...
ISInventoryPaneContextMenu.doClothingPatchMenu = function(player, clothing, context)
    ...
    if (SandboxVars.TOC.EnableMetalPatches) then
        local fabric4 = playerObj:getInventory():getItemFromType("MetalStrips", true, true)
    end


    if not thread or not needle or (not fabric1 and not fabric2 and not fabric3 and not fabric4 and SandboxVars.TOC.EnableMetalPatches) then
        ...
        return
    elseif not thread or not needle or (not fabric1 and not fabric2 and not fabric3 and SandboxVars.TOC.EnableMetalPatches ~= false) then 
        local patchOption = context:addOption(getText("ContextMenu_Patch"))
        patchOption.notAvailable = true
        local tooltip = ISInventoryPaneContextMenu.addToolTip()
        tooltip.description = getText("ContextMenu_CantRepair")
